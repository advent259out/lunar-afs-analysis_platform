...
void sdr_pvt_set_poscsv(sdr_pvt_t *pvt, const char *path)
{
    if (!pvt) return;
    if (pvt->pos_csv) {
        fclose(pvt->pos_csv);
    ...
    if (path && *path) {
        ...
        if (!fp) {
            fprintf(stderr, "ERROR: failed to open position CSV: %s\n", path);
            return;
        }
        pvt->pos_csv = fp;
        fprintf(fp, "time_s,lat_deg,lon_deg,height_m,x_m,y_m,z_m,stat,ns,nsat\n");
        fflush(fp);
    }
}

void sdr_pvt_log_poscsv(sdr_pvt_t *pvt, double time, const sol_t *sol, int nsat)
{
    if (!pvt || !pvt->pos_csv || !sol) return;
    if (sol->stat <= 0) return;
    double llh[3];
    ecef2pos(sol->rr, llh);
    fprintf(pvt->pos_csv,
        "%.3f,%.9f,%.9f,%.4f,%.3f,%.3f,%.3f,%d,%d,%d\n",
        time, llh[0] * R2D, llh[1] * R2D, llh[2],
        sol->rr[0], sol->rr[1], sol->rr[2], sol->stat, sol->ns, nsat);
    fflush(pvt->pos_csv);
}

void sdr_pvt_set_satcsv(sdr_pvt_t *pvt, const char *path)
{
    if (!pvt) return;
    if (pvt->sat_csv) {
        fclose(pvt->sat_csv);
    ...
    if (path && *path) {
        FILE *fp = fopen(path, "w");
        if (!fp) {
            fprintf(stderr, "ERROR: failed to open satellite CSV: %s\n", path);
            return;
        }
        pvt->sat_csv = fp;
        fprintf(fp, "time_s,sat_id,sys,prn,x_m,y_m,z_m,clk_m\n");
        fflush(fp);
    }
}

void sdr_pvt_log_satcsv(sdr_pvt_t *pvt, double time, const sol_t *sol)
{
    if (!pvt || !pvt->sat_csv || !sol) return;
    if (!pvt->obs || !pvt->nav) return;
    int used[MAXSAT] = {0};
    for (int i = 0; i < pvt->obs->n; i++) {
        const obsd_t *data = pvt->obs->data + i;
        if (data->sat < 1 || data->sat > MAXSAT) continue;
        if (used[data->sat - 1]) continue;
        double rs[6] = {0}, dts[2] = {0}, var;
        int svh = 0;
        int have_satpos = satpos(data->time, data->time, data->sat, EPHOPT_BRDC, pvt->nav, rs, dts, &var, &svh);
        if (!have_satpos) {
            int is_afs = 0;
            for (int j = 0; j < NFREQ + NEXOBS; j++) {
                uint8_t c = data->code[j];
                if (c == CODE_L9D || c == CODE_L9P) { is_afs = 1; break; }
            }
            if (is_afs) {
                have_satpos = sdr_pvt_afs_satpos(data->sat, data->time, rs, dts);
            }
        }
        if (!have_satpos) {
            continue;
        ...
        used[data->sat - 1] = 1;
        char satid[16];
        satno2id(data->sat, satid);
        int prn = 0;
        int sys = satsys(data->sat, &prn);
        char sysc = '?';
        switch (sys) {
        case SYS_GPS: sysc = 'G'; break;
        case SYS_GLO: sysc = 'R'; break;
        case SYS_GAL: sysc = 'E'; break;
        case SYS_QZS: sysc = 'Q'; break;
        case SYS_CMP: sysc = 'C'; break;
        case SYS_IRN: sysc = 'I'; break;
        case SYS_SBS: sysc = 'S'; break;
        default: sysc = '?'; break;
        }
        double clk = dts[0] * CLIGHT;
        fprintf(pvt->sat_csv, "%.3f,%s,%c,%d,%.9f,%.9f,%.9f,%.6f\n",
            time, satid, sysc, prn, rs[0], rs[1], rs[2], clk);
    }
    fflush(pvt->sat_csv);
}
...
static void update_sol(sdr_pvt_t *pvt)
{
    ...
    if (pntpos(pvt->obs->data, pvt->obs->n, pvt->nav, &opt, pvt->sol, NULL,
        ...
        sdr_pvt_log_poscsv(pvt, time, pvt->sol, pvt->obs->n);
...
void sdr_pvt_udsol(sdr_pvt_t *pvt, int64_t ix)
{
    ...
    if (pvt->ix > 0 && (pvt->nch >= pvt->rcv->nch ||
        ...
        // Detect AFS observations in any frequency slot (not only [0])
        for (int i = 0; i < pvt->obs->n && !pvt_afs; i++) {
            for (int j = 0; j < NFREQ + NEXOBS; j++) {
                uint8_t c = pvt->obs->data[i].code[j];
                if (c == CODE_L9D || c == CODE_L9P) { pvt_afs = 1; break; }
            }
        }
...
void sdr_pvt_solstr(sdr_pvt_t *pvt, char *buff)
{
    ...
    // snapshot ECEF velocity while holding lock to avoid races
    double v_ecef[3] = { pvt->sol->rr[3], pvt->sol->rr[4], pvt->sol->rr[5] };
    pthread_mutex_unlock(&pvt->mtx);
    
    tstr[4] = tstr[7] = '-';
    // include NEU velocity components (m/s)
    // sanitize potential non-finite inputs from solver
    for (int i = 0; i < 3; i++) {
        if (!isfinite(v_ecef[i])) v_ecef[i] = 0.0;
        if (!isfinite(pos[i]))    pos[i]    = 0.0;
    }
    double enu[3] = {0};
    ecef2enu(pos, v_ecef, enu); // enu = [E,N,U]
    // guard against non-finite outputs
    for (int i = 0; i < 3; i++) if (!isfinite(enu[i])) enu[i] = 0.0;
    double vn = enu[1], ve = enu[0], vu = enu[2];
    sprintf(buff, "%23s %11.7f %12.7f %8.2f %2d/%2d %s VN=%+6.1f VE=%+6.1f VU=%+6.1f",
        tstr, pos[0] * R2D, pos[1] * R2D, pos[2], pvt->sol->ns, pvt->nsat,
        stat ? "FIX" : "---", vn, ve, vu);
}
