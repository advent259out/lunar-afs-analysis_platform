...
typedef struct {                // signal tracking type 
    ...
    int coh_k;                  // coherent accumulation factor (code periods)
    int coh_cnt;                // current accumulation count
    sdr_cpx_t C_acc[SDR_N_CORR]; // accumulated correlations
} sdr_trk_t;
...
typedef struct {                // SDR PVT type
    ...
    struct sdr_rcv_tag *rcv;    // pointer to SDR receiver
    pthread_mutex_t mtx;        // lock flag
    FILE *pos_csv;              // optional position CSV output
    FILE *sat_csv;              // optional satellite position CSV output
} sdr_pvt_t;
