...
#define B_PLL      15.0      // band-width of PLL filter (Hz)
...
// AFSP coherent integration (tracking) factor (number of code periods)
// 1: default (2 ms for AFSP), 5: ~10 ms coherent accumulation
int sdr_afsp_coh_k = 1;
...
static void trk_init(sdr_trk_t *trk)
{
    ...
    trk->coh_k = 1;
    trk->coh_cnt = 0;
    memset(trk->C_acc, 0, sizeof(sdr_cpx_t) * SDR_N_CORR);
}
    ...
    double cn0)
{
    ...
    if (!strcmp(ch->sig, "AFSP")) {
        ch->trk->coh_k = sdr_afsp_coh_k > 1 ? sdr_afsp_coh_k : 1;
    }
}
...
static void track_sig(sdr_ch_t *ch, double time, const sdr_buff_t *buff, int ix)
{
    ...
    // Update prompt history and advance lock/TOW for this code period first
    sdr_add_buff(ch->trk->P, SDR_N_HIST, ch->trk->C[0], sizeof(sdr_cpx_t));
    update_tow(ch, ch->T);
    ch->lock++;

    // Sync and remove secondary code BEFORE coherent accumulation
    if (ch->len_sec_code >= 2 && ch->lock * ch->T >= T_NPULLIN) {
        sync_sec_code(ch, ch->len_sec_code);
    }

    // Coherent accumulation for AFSP tracking if requested (after de-spreading)
    if (!strcmp(ch->sig, "AFSP") && ch->trk->coh_k > 1) {
        if (ch->trk->sec_sync <= 0) {
            // not synchronized to secondary code yet: do not accumulate
            ch->trk->coh_cnt = 0;
            memset(ch->trk->C_acc, 0, sizeof(sdr_cpx_t) * SDR_N_CORR);
        }
        else {
            for (int ii = 0; ii < SDR_N_CORR; ii++) {
                ch->trk->C_acc[ii][0] += ch->trk->C[ii][0];
                ch->trk->C_acc[ii][1] += ch->trk->C[ii][1];
            }
            ch->trk->coh_cnt++;
            ...
            // finalize coherent sum: replace current C by accumulated value
            for (int ii = 0; ii < SDR_N_CORR; ii++) {
                ch->trk->C[ii][0] = ch->trk->C_acc[ii][0];
                ch->trk->C[ii][1] = ch->trk->C_acc[ii][1];
            }
            memset(ch->trk->C_acc, 0, sizeof(sdr_cpx_t) * SDR_N_CORR);
            ch->trk->coh_cnt = 0;
        }
