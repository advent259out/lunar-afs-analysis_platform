...
rect1  = [0.080, 0.560, 0.560, 0.400]  # give a bit more headroom
rect2  = [0.652, 0.560, 0.280, 0.400]
rect3  = [0.080, 0.188, 0.840, 0.340]
...
def on_close(event):
    global plot_ena
    plot_ena = False
    event.canvas.figure.savefig('/mnt/e/Project/PocketSDR-AFS-main/PocketSDR-AFS-main/python/afs_track_final.png', dpi=200)  # 结束时保存
    save_history_plots()
...
def update(fig, ch=None):
    ...
    if not ch:
        ...
        fig.savefig('/mnt/e/Project/PocketSDR-AFS-main/PocketSDR-AFS-main/python/afs_track.png', dpi=200)  # 这里改成你想保存的路径
...
def draw_corr(fig, rect, opt):
    ...
    ax.grid(True, lw=0.3)
    ax.set_xticks([])
    # Add headroom to avoid clipping labels/peaks
    ypad = 1.15
    ax.set_ylim([0.0 if opt[0] else -0.1 * ypad, opt[4] * ypad])
...
def draw_IQ(fig, rect, opt):
    ...
    ax.set_aspect('equal')
    ypad = 1.15
    ax.set_xlim([-opt[4] * ypad, opt[4] * ypad])
    ax.set_ylim([-opt[4] * ypad, opt[4] * ypad])
...
def draw_time(fig, rect, opt):
    ax = fig.add_axes(rect)
    set_axcolor(ax, lc)
    # Add headroom to show text fully
    ypad = 1.15
    ax.set_ylim(-opt[4] * ypad, opt[4] * ypad)
    ax.grid(True, lw=0.3)
    ax.set_xticks([])
    ...
    for i in range(0 if N <= 4 else N - 4, N):
        ...
        for j in range(len(ch.nav.data[i][1])):
            ...
            if j >= 42:
                ...
                break
        txt += '\n'
    # nav text is the second text object if it exists, otherwise add one
    if len(ax.texts) >= 2:
        ax.texts[1].set_text(txt)
    else:
        ax.text(0.01, 0.9, txt, ha='left', va='top', color=fc,
                transform=ax.transAxes, fontname='monospace')
    draw_axis(ax)

def collect_discriminators(ch):
    t = ch.time
    pll_raw_sample = ch.trk.PLL_raw[-1]
    if not np.isnan(pll_raw_sample):
        pll_raw_hist.append((t, float(pll_raw_sample)))
    pll_filt_sample = ch.trk.PLL_filt[-1]
    if not np.isnan(pll_filt_sample):
        pll_filt_hist.append((t, float(pll_filt_sample)))
    dll_raw_sample = ch.trk.DLL_err[-1]
    if not np.isnan(dll_raw_sample):
        dll_raw_hist.append((t, float(dll_raw_sample)))
    dll_filt_sample = ch.trk.DLL_filt[-1]
    if not np.isnan(dll_filt_sample):
        dll_filt_hist.append((t, float(dll_filt_sample)))
    if len(ch.trk.C) >= 3:
        prompt_hist.append((t, float(abs(ch.trk.C[0]))))
        early_hist.append((t, float(abs(ch.trk.C[1]))))
        late_hist.append((t, float(abs(ch.trk.C[2]))))

def save_history_plots(output_dir=None, total_time=None):
    ...
    inferred = 0.0
    for hist in (pll_raw_hist, pll_filt_hist, dll_raw_hist, dll_filt_hist):
        if hist:
            inferred = max(inferred, hist[-1][0])
    duration = total_time if total_time is not None else inferred
    if duration <= 0.0:
        return

    if pll_raw_hist or pll_filt_hist:
        fig, ax = plt.subplots()
        ax.set_title('PLL Discriminator History')
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('Discriminator (deg)')
        ax.grid(True, lw=0.3)
        if pll_raw_hist:
            t_raw, v_raw = zip(*pll_raw_hist)
            ax.plot(t_raw, v_raw, color='magenta', lw=0.6, label='raw')
        if pll_filt_hist:
            t_filt, v_filt = zip(*pll_filt_hist)
            ax.plot(t_filt, v_filt, color='cyan', lw=0.8, label='filtered')
        ax.set_xlim(0.0, duration)
        ax.legend(loc='upper right')
        fig.savefig(output_dir / 'pll_discriminator_history.png', dpi=200)
        plt.close(fig)

    if dll_raw_hist or dll_filt_hist:
        fig, ax = plt.subplots()
        ax.set_title('DLL Discriminator History')
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('DLL Error (s)')
        ax.grid(True, lw=0.3)
        if dll_raw_hist:
            t_raw, v_raw = zip(*dll_raw_hist)
            ax.plot(t_raw, v_raw, color='green', lw=0.6, label='raw')
        if dll_filt_hist:
            t_filt, v_filt = zip(*dll_filt_hist)
            ax.plot(t_filt, v_filt, color='orange', lw=0.8, label='filtered')
        ax.set_xlim(0.0, duration)
        ax.legend(loc='upper right')
        fig.savefig(output_dir / 'dll_discriminator_history.png', dpi=200)
        plt.close(fig)
    ...
    if prompt_hist:
        ...
        ax.grid(True, lw=0.3)
        tp, vp = zip(*prompt_hist)
        ax.scatter(tp, vp, s=2, color='royalblue', label=r'$\sqrt{I_P^2+Q_P^2}$')
        if early_hist:
            te, ve = zip(*early_hist)
            ax.scatter(te, ve, s=2, color='darkorange', label=r'$\sqrt{I_E^2+Q_E^2}$')
        if late_hist:
            tl, vl = zip(*late_hist)
            ax.scatter(tl, vl, s=2, color='firebrick', label=r'$\sqrt{I_L^2+Q_L^2}$')
        ax.set_xlim(0.0, duration)
        ax.legend(loc='upper right')
        fig.savefig(output_dir / 'correlation_results.png', dpi=200)
        plt.close(fig)
...
def ch_status(ch):
    ...
    txt1 = 'COFF=%10.7f ms DOP=%8.1f Hz ADR=%10.1f cyc C/N0=%5.1f dB-Hz SNR=%5.1f dB' % (
        ch.coff * 1e3, ch.fd, ch.adr, ch.cn0, snr_db)
